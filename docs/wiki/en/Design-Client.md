...
### `namespace(&self, namespace: &str) -> Result<wasm_bindgen::JsValue, Error>` (WASM)

**For WASM targets only.**

Gets the Javascript representation of the mapped namespace format. This method:

1. Looks up or creates a `Cache` for the specified namespace.
2. Retrieves the current configuration value from the cache.
3. Mapped as `JsValue`:
   - Properties format → returns a JS `Properties` class instance.
   - JSON format → returns a raw JS object containing the configuration data.
   - YAML format → returns a raw JS string.
   - Text format → returns a raw JS string.
...
- `JsValue`: Mapped JS object/value representation
...
- **Returns**: JS-wrapped Namespace representation (e.g. `Properties` class or raw JS object) from `namespace()` method
- **Caching**: Memory-only caching (Node.js) or persistent `localStorage` caching (browser)
- **Background Tasks**: Uses `spawn_local` without direct handles
- **Event Listeners**: Client-level event listeners with JavaScript interop

...
- **Signal Handling & Limitations**:
  - Traps `SIGINT` (exit 130) and `SIGTERM` (exit 143), terminates active child processes, captures diagnostic logs, and cleans up Docker resources via the protected lifecycle state machine (`LIFECYCLE_PHASE="work" | "spawning" | "cleanup" | "finished"`).
  ...
  - Signal deferral and registration: signals received during the `spawning` phase are deferred until child process registration is completed, after which the work path aborts with `exit "$INTERRUPTED_STATUS"` so cleanup can run. Signals received during the `cleanup` phase are deferred without aborting the shell, resetting deadlines, or restarting `down`, ensuring the current process group is boundedly supervised and reaped. Signals received during the terminal `finished` phase immediately exit with `INTERRUPTED_STATUS`.

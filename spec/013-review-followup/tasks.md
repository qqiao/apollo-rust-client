...
- **Evidence:**
  ...
  - Verified: `node --test tests/tooling/doc-examples.test.mjs` passed all 13 tests (including end-to-end LF/CRLF `validateInventory` fixture test and invalid marked opener regression). `node scripts/check-doc-examples.mjs` compiled all 4 examples under both native-tls and rustls. `node scripts/check-doc-links.mjs` checked 441 links across 82 files with 0 broken links. All 45 tooling unit tests passed in `scripts/test.sh fast`.

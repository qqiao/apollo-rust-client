...
The feature specs define outcomes; this table records current source/test mappings and explicitly labeled historical evidence. The [current audit](../verification-2026-09-13.md) records the latest verification and limitations. IDs are qualified by the feature containing them. `AC-*` references point to acceptance scenarios in that feature.
...
|---|---|---|
| FR-001 | AC-001, AC-003 | Test + source: `real_apollo_formats_and_identity`; source `Client::namespace`. |
...
| FR-004 | AC-007, AC-008 | Test + source: `authenticated_request_contains_required_headers`, `test_sign_with_path`, `test_sign_url`, `real_apollo_grayscale`; unsigned rejection depends on the server fixture, not client authorization. |
...
| FR-007 | AC-010 | Test + source: `getters_accept_string_number_and_boolean_scalars`, `real_apollo_formats_and_identity`; complete conversion matrices are not claimed. |
| FR-008 | AC-011, AC-013 | Test + source: JSON/YAML `test_json_to_object`, `test_yaml_to_object`, `real_apollo_formats_and_identity`, `preserves_yaml_1_1_boolean_scalars`; malformed-content/type-mismatch branches also source-inspected. |
| FR-009 | AC-012, AC-013 | Partial: `real_apollo_formats_and_identity` covers successful text; missing/non-string content and exact preservation branches source-inspected. |
...
| FR-004 | AC-004, AC-016 | Partial: `zero_ttl_serves_cached_data_and_revalidates` covers replacement; timestamp renewal on identical content is source-inspected. AC-016 still lacks a dedicated unchanged-response freshness-renewal test. |
| FR-005 | AC-005 | Test + source: `stale_persistent_value_falls_back_and_reports_refresh_error`, WASM `test_wasm_local_storage_caching`; `real_apollo_preload_and_persistence` also creates a second matching client; a server-unavailable two-client restart is not isolated by that real-server test. |
...
| Requirement | Acceptance | Evidence and remaining coverage |
|---|---|---|
| FR-001 | AC-002, AC-007 | Test + source: `real_apollo_release_refresh_and_listener`; append-only registration behavior source-inspected. |
...
| FR-002 | AC-002, AC-007 | Test + source: `namespace_wasm_preserves_properties_class_api`, WASM lifecycle tests; generated bindings are rebuilt by the real Node/WASM integration suite. |
| FR-003 | AC-002 | Partial: `namespace_wasm_preserves_properties_class_api`, `wasm_conversion_returns_a_structured_value`; `real_apollo_wasm_formats_and_identity` asserts a bigint result (42n); full integer-boundary coverage is not claimed. |
...
| FR-010 | AC-012 | Test + source: `scripts/wasm_api_smoke.js` executes canonical ownership guidance against fresh generated bindings, including application and constructor errors. Real WASM tests release Properties in finally blocks. No quantitative leak certification is claimed. |
...
| FR-009 | AC-013 | Source: `.github/workflows/rust.yml` configures the same `scripts/test.sh` lifecycle, preflight, public-runner execution without private secrets, and artifact upload. Actual PR/main/fork execution and failure-artifact verification are not established in this audit. |
...
## Historical feature-005 execution record
...
| [013](../013-review-followup/tasks.md) | 009 lifecycle supervision (V1); 011 fence tracking (V2); 006–012 status (V3) | 013/AC-001–010 | **Verified**: Protected lifecycle phase tracking and first-signal status handling during automatic teardown and recovery; 4 new deterministic signal regressions in `tests/tooling/apollo-lifecycle.test.mjs` (21 tests pass). CommonMark fence tracking in `scripts/check-doc-examples.mjs`, unclosed fence repaired in `docs/wiki/en/Error-Handling.md`; 3 new fence regressions in `tests/tooling/doc-examples.test.mjs` (11 tests pass); 4 examples compiled under native-tls & rustls. Reconciled all entry-point and specification completion statuses. Verified via `scripts/test.sh fast` and `scripts/test.sh integration --suite native`. |
| [014](../014-copilot-remediation/tasks.md) | 009/FR-001–006, 010/FR-006, 011/FR-001–006, 013/FR-007; PR #133 Copilot review rounds | 014/AC-001–010, R3-1–R3-5, Round 4–5 follow-ups | **Verified**: Shared URL sanitization in `scripts/apollo-fixtures.mjs` redacting basic auth and sensitive query parameters across case/delimiter forms (F1, F9); CommonMark-compliant fence, destination, and code-span parsing in `scripts/check-doc-links.mjs` (F2–F4, F10, Round 4); child process registration and process-group supervision in `scripts/apollo-test-lifecycle.sh` (F5–F8) and diagnostic capture failure tracking; compiler diagnostics in `scripts/check-doc-examples.mjs` (F11); spec/doc consistency and link maintenance (F12–F17, Round 5). Tooling tests pass via `node --test tests/tooling/*.test.mjs` and `scripts/test.sh fast`. |
...
## Current scoped amendment coverage

| Feature requirement | Scenario | Current evidence |
|---|---|---|
| 001/FR-013 | 001/AC-018 | Public `CacheError` re-export; four external consumer tests run under default TLS and Rustls. |
| 002/FR-013 | 002/AC-017 | Five persisted restoration tests in `src/cache.rs`, including the controlled old/new interleaving; 006/AC-001–007 mapping above. |
| 004/FR-010 | 004/AC-013 | Fresh generated-binding ownership smoke includes constructor failure and application error preservation. |

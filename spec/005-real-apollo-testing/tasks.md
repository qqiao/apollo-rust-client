...
- [ ] Both native runtime feature configurations pass all format/identity/auth/grayscale cases, with no cached-success shortcut in negative cases.
- [ ] Required missing environment or zero selected tests fails; ordinary fast mode still runs without a server.
- [ ] Integration tests compile/lint for applicable targets without new production API exposure or unreviewed dependencies.
